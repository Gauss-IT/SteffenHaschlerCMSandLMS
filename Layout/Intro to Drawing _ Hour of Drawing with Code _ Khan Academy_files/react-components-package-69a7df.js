((this.KAdefineTiming = this.KAdefineTiming || {})["react-components-package.js"] = {}).startTime = +(new Date);
...
KAdefine("third_party/javascript-khansrc/react-components/js/blur-input.jsx", function(require, module, exports) {
var React=require("react")
var BlurInput=React.createClass({displayName:"BlurInput",propTypes:{value:React.PropTypes.string.isRequired,onChange:React.PropTypes.func.isRequired},getInitialState:function(){return{value:this.props.value}},render:function(){return React.createElement("input",React.__spread({},this.props,{type:"text",value:this.state.value,onChange:this.handleChange,onBlur:this.handleBlur}))},componentWillReceiveProps:function(e){this.setState({value:e.value})},handleChange:function(e){this.setState({value:e.target.value})},handleBlur:function(e){this.props.onChange(e.target.value)}})
...
var _=require("underscore")
var interpolateStringToArray=function(r,e){e=e||{}
var t=r.split(/%\(([\w_]+)\)s/g)
for(var n=1;n<t.length;n+=2){var i=e[t[n]]
t[n]=_.isUndefined(i)?"%("+t[n]+")s":i}return t}
var $_=function(r,e){if(arguments.length!==2||!_.isString(e)){return"<$_> must have exactly one child, which must be a string"}return interpolateStringToArray(e,r)}
module.exports=$_
});
...
document.body.removeChild(this._layer)},_renderLayer:function(){React.render(this.renderLayer(),this._layer)
...
KAdefine("third_party/javascript-khansrc/react-components/js/tooltip.jsx", function(require, module, exports) {
var React=require("react")
var _=require("underscore")
var zIndex=10
var Triangle=React.createClass({displayName:"Triangle",propTypes:{color:React.PropTypes.string.isRequired,left:React.PropTypes.number.isRequired,top:React.PropTypes.number.isRequired,width:React.PropTypes.number.isRequired,height:React.PropTypes.number.isRequired,horizontalDirection:React.PropTypes.oneOf(["left","right"]).isRequired,verticalDirection:React.PropTypes.oneOf(["top","bottom"]).isRequired},render:function(){var t,e,o,r
var i=this.props.width+"px solid transparent"
if(this.props.horizontalDirection==="right"){t=i}else{e=i}var p=this.props.height+"px solid "+this.props.color
if(this.props.verticalDirection==="top"){o=p}else{r=p}return React.createElement("div",{style:{display:"block",height:0,width:0,position:"absolute",left:this.props.left,top:this.props["top"],borderLeft:t,borderRight:e,borderTop:o,borderBottom:r}})}})
var TooltipArrow=React.createClass({displayName:"TooltipArrow",propTypes:{position:React.PropTypes.string,visibility:React.PropTypes.string,left:React.PropTypes.number,top:React.PropTypes.number,color:React.PropTypes.string.isRequired,border:React.PropTypes.string.isRequired,width:React.PropTypes.number.isRequired,height:React.PropTypes.number.isRequired,horizontalDirection:React.PropTypes.oneOf(["left","right"]).isRequired,verticalDirection:React.PropTypes.oneOf(["top","bottom"]).isRequired},getDefaultProps:function(){return{position:"relative",visibility:"visible",left:0,top:0}},render:function(){var t=this.props.horizontalDirection==="right"
var e=this.props.verticalDirection==="top"
var o=e?0:1
var r=e?0:-1
...
var HORIZONTAL_ALIGNMNENTS={left:{tooltipLeft:0,arrowLeft:function(t){return 0}},right:{tooltipLeft:"-100%",arrowLeft:function(t){return-t-2}}}
var Tooltip=React.createClass({displayName:"Tooltip",propTypes:{show:React.PropTypes.bool.isRequired,className:React.PropTypes.string,arrowSize:React.PropTypes.number,borderColor:React.PropTypes.string,verticalPosition:React.PropTypes.oneOf(_.keys(VERTICAL_CORNERS)),horizontalPosition:React.PropTypes.oneOf(_.keys(HORIZONTAL_CORNERS)),horizontalAlign:React.PropTypes.oneOf(_.keys(HORIZONTAL_ALIGNMNENTS)),children:React.PropTypes.arrayOf(React.PropTypes.element).isRequired},getDefaultProps:function(){return{className:"",arrowSize:10,borderColor:"#ccc",verticalPosition:"bottom",horizontalPosition:"left",horizontalAlign:"left"}},getInitialState:function(){return{height:null}},componentWillReceiveProps:function(){this.setState({height:null})},render:function(){var t=this.props.verticalPosition==="top"
return React.createElement("span",null,t&&this._renderToolTipDiv(t),React.createElement("div",null,_.first(this.props.children)),!t&&this._renderToolTipDiv())},_renderToolTipDiv:function(t){var e=_.extend({},HORIZONTAL_CORNERS[this.props.horizontalPosition],HORIZONTAL_ALIGNMNENTS[this.props.horizontalAlign],VERTICAL_CORNERS[this.props.verticalPosition])
var o
var r
if(t){o=React.createElement(TooltipArrow,{verticalDirection:"top",horizontalDirection:this.props.horizontalAlign,position:"absolute",color:"white",border:this.props.borderColor,left:e.arrowLeft(this.props.arrowSize),top:-this.props.arrowSize+2,width:this.props.arrowSize,height:this.props.arrowSize,zIndex:zIndex})
r=React.createElement(TooltipArrow,{verticalDirection:"top",horizontalDirection:this.props.horizontalAlign,visibility:"hidden",color:"white",border:this.props.borderColor,left:e.arrowLeft(this.props.arrowSize),top:-1,width:this.props.arrowSize,height:this.props.arrowSize,zIndex:zIndex})}else{o=React.createElement(TooltipArrow,{verticalDirection:"bottom",horizontalDirection:this.props.horizontalAlign,color:"white",border:this.props.borderColor,left:e.arrowLeft(this.props.arrowSize),top:-1,width:this.props.arrowSize,height:this.props.arrowSize,zIndex:zIndex})
r=null}return React.createElement("div",{style:{position:"relative",height:0,display:this.props.show?"block":"none"}},React.createElement("div",{ref:"tooltipContainer",className:"tooltipContainer",style:{position:"absolute",height:this.state.height||undefined,left:e.targetLeft}},o,React.createElement("div",{className:this.props.className,ref:"tooltipContent",style:{position:"relative",top:e["top"],left:e.tooltipLeft,border:"1px solid "+this.props.borderColor,WebkitBoxShadow:"0 1px 3px "+this.props.borderColor,MozBoxShadow:"0 1px 3px "+this.props.borderColor,boxShadow:"0 1px 3px "+this.props.borderColor,zIndex:zIndex-1}},_.rest(this.props.children)),r))},componentDidMount:function(){this._updateHeight()},componentDidUpdate:function(){this._updateHeight()},_updateHeight:function(){var t=this.refs.tooltipContainer.getDOMNode().offsetHeight
if(t!==this.state.height){this.setState({height:t})}}})
module.exports=Tooltip
});
this.KAdefineTiming["react-components-package.js"].endTime = +(new Date);
